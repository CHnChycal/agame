...
void Bag::showGoods(int* goodsId, int* num) {
	for (int i = 0; goodsId[i] >= 0; i++)
		cout << i + 1 << "." << setw(10) << left << goods[goodsId[i]].getName() << "x" << num[i] << endl;
	cout << endl;
}

void Bag::showGoods(int goodsId, int num) {
	cout << 1 << '.' << setw(10) << left << goods[goodsId].getName() << "x" << num << endl;
	cout << endl;
}
...
void Bag::saveBag() {
	...
	for (iter = bags.begin(); iter != bags.end(); ++iter) {
		int i = 0;
		saveGoods[i][0] = goods[iter->first].getGoodsId();//注意
...
/*int** Bag::getSaveBag(){
	int **saveGoodsArray[i]=new int[2];
	for(int i= 0;i<20;i++) {
...
bool Bag::isUseDrug() {
	map<int, int>::iterator iter;
	for (iter = bags.begin(); iter != bags.end(); ++iter) {
		if (goods[iter->first].getGoodsId() == 4)
			return true;
	}
	return false;
}
...
int Bag::getWhichGoodsId(int whichGoods) {  //返回要选择物品的Id
	map<int, int>::iterator iter;
	iter = bags.begin();
